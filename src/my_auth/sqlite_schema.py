...
def _canonical_layout_diagnostics(
    ...
    indexes = {
        ...
        for row in _fetchall(connection.execute("PRAGMA index_list(passkey_credentials)"))
    }
    if ("idx_passkey_credentials_user_id", 0) not in indexes:
        diagnostics.append("missing index (idx_passkey_credentials_user_id)")
    indexes = {
        (str(row[1]), _as_int(row[2]))
        for row in _fetchall(connection.execute("PRAGMA index_list(passkey_challenges)"))
    }
    ...
    if not any(
        _as_int(row[2]) == 1
        and [item[2] for item in _fetchall(connection.execute(f"PRAGMA index_info({row[1]})"))]
        == ["user_handle"]
    ...
    foreign_keys = [
        tuple(row[2:6])
        for row in _fetchall(connection.execute("PRAGMA foreign_key_list(passkey_credentials)"))
    ]
    if foreign_keys != [("passkey_users", "user_id", "user_id", "NO ACTION")]:
...
def inspect_sqlite_schema(connection: sqlite3.Connection) -> SQLiteSchemaInspection:
    ...
    if legacy_flow_key:
        expected_legacy = {
            ...
            "passkey_challenges": {
                "flow_key", "kind", "challenge", "expires_at", "user_id",
        ...
        if missing:
            return SQLiteSchemaInspection(
                "unsupported", diagnostics=tuple(f"missing columns ({item})" for item in missing)
            )
        if "my_auth_schema" in tables:
            return SQLiteSchemaInspection(
                "unsupported", diagnostics=("legacy layout has schema metadata",)
            )
        return SQLiteSchemaInspection("legacy", diagnostics=("legacy flow_key challenge layout",))
...
def ensure_sqlite_schema(connection: sqlite3.Connection) -> None:
    """Create/stamp the canonical schema; never migrate an existing layout."""
    ...
    if state.state == "empty":
        _ = connection.executescript(sqlite_schema_sql())
    ...
    if state.state == "current":
        return
    if state.state == "canonical_unversioned":
        _ = connection.execute(
            "CREATE TABLE IF NOT EXISTS my_auth_schema (schema_version INTEGER NOT NULL)"
        )
        _ = connection.execute("DELETE FROM my_auth_schema")
...
def migrate_sqlite_schema(connection: sqlite3.Connection) -> SQLiteSchemaInspection:
    ...
    try:
        ...
        for row in _fetchall(connection.execute("SELECT user_id,user_handle,name,display_name FROM passkey_users")):
            _ = connection.execute("INSERT INTO passkey_users_v2 VALUES(?,?,?,?)", (row[0], _b64(row[1]), row[2], row[3]))

        credential_columns = _columns(connection, "passkey_credentials")
        required = {"credential_id", "user_id", "public_key", "sign_count", "created_at"}
        if not required.issubset(credential_columns):
            raise UnsupportedSQLiteSchema("legacy credentials missing required columns")
        optional = [name for name in ("transports", "device_type", "backed_up", "label") if name in credential_columns]
        select = "credential_id,user_id,public_key,sign_count"
        if optional:
            select += "," + ",".join(optional)
        select += ",created_at"
        for row in _fetchall(connection.execute(f"SELECT {select} FROM passkey_credentials")):
            values = list(row)
            ...
            created_at = values[-1]
            _ = connection.execute("INSERT INTO passkey_credentials_v2 VALUES(?,?,?,?,?,?,?,?,?)", (credential_id, user_id, public_key, sign_count, fields.get("transports"), fields.get("device_type"), fields.get("backed_up"), fields.get("label"), created_at))
        challenge_columns = _columns(connection, "passkey_challenges")
        key_column = "flow_key" if "flow_key" in challenge_columns else "key"
        for row in _fetchall(connection.execute(f"SELECT {key_column},kind,challenge,expires_at,user_id,user_handle,user_name,user_display_name FROM passkey_challenges")):
            values = list(row)
            values[5] = _b64(values[5])
            _ = connection.execute("INSERT INTO passkey_challenges_v2 VALUES(?,?,?,?,?,?,?,?)", values)
        ...
        for source in source_counts:
            count_row = _fetchone(connection.execute(f"SELECT COUNT(*) FROM {source}_v2"))
            if count_row is None:
                raise SQLiteSchemaError("missing target row count")
            target_counts[source] = _as_int(count_row[0])
        mismatched = [f"{table}: source={source_counts[table]} target={target_counts[table]}" for table in source_counts if source_counts[table] != target_counts[table]]
        if mismatched:
            raise SQLiteSchemaError("migration row count validation failed: " + "; ".join(mismatched))
        ...
        _ = connection.execute("ALTER TABLE passkey_credentials_v2 RENAME TO passkey_credentials")
        _ = connection.execute("ALTER TABLE passkey_challenges_v2 RENAME TO passkey_challenges")
        _ = connection.execute("CREATE INDEX idx_passkey_credentials_user_id ON passkey_credentials(user_id)")
        _ = connection.execute("CREATE INDEX idx_passkey_challenges_expires_at ON passkey_challenges(expires_at)")
        _ = connection.execute("CREATE TABLE my_auth_schema (schema_version INTEGER NOT NULL)")
